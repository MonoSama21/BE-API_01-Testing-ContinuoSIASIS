...
Background:
    ...
    * def requestExitoso = read("classpath:resources/functional/request/Auth/requestProfesorPrimaria.json")
    * def schemas = read('classpath:resources/functional/schema/LoginSuccessResponse.json')

# =======================================================================
# 1. 🔵 SMOKE Y HAPPY PATH 
# =======================================================================
@contract @smoke @post @happy-path @tokenProfesorPrimaria
Scenario: Validar que el servicio de login responde correctamente para el rol profesor primaria
...
Scenario: Validar que el servicio de login devuelve una respuesta correcta cuando se accede por el rol profesor primaria
    Given path "api/login/profesor-primaria"
    And request requestExitoso
    When method POST
    Then status 200
...
Scenario: Validar headers de respuesta al acceder por el rol profesor primaria
    Given path "api/login/profesor-primaria"
    And request requestExitoso
    When method POST
    Then status 200
...
Scenario Outline: Validar <descripcion> con rol profesor primaria
    Given path "api/login/profesor-primaria"
    And request <body>
    When method POST
    Then status 400
...
Scenario Outline: Validar inyección en cabeceras HTTP al acceder por el rol profesor primaria <descripcion>
    Given path "api/login/profesor-primaria"
