@postLoginRolTutorSecundaria @login
Feature: Login de usuario tutor secundaria - Pruebas de Contrato API

Background:
    * url urlBase
    * header Content-Type = "application/json"
    * def requestExitoso = read("classpath:resources/functional/request/Auth/requestProfesorTutorSecundaria.json")
    * def schemas = read('classpath:resources/functional/schema/LoginSuccessResponse.json')

# =======================================================================
# 1. 🔵 SMOKE Y HAPPY PATH 
# =======================================================================
@contract @smoke @post @happy-path @tokenTutorSecundaria
Scenario: Validar que el servicio de login responde correctamente para el rol tutor secundaria
...
Scenario: Validar que el servicio de login devuelve una respuesta correcta cuando se accede por el rol tutor secundaria
    Given path "api/login/profesor-tutor-secundaria"
    And request requestExitoso
    When method POST
    Then status 200
...
Scenario: Validar headers de respuesta al acceder por el rol tutor secundaria
    Given path "api/login/profesor-tutor-secundaria"
    And request requestExitoso
    When method POST
    Then status 200
...
Scenario Outline: Validar <descripcion> con rol tutor secundaria
    Given path "api/login/profesor-tutor-secundaria"
    And request <body>
    When method POST
    Then status 400
...
Scenario Outline: Validar inyección en cabeceras HTTP al acceder por el rol tutor secundaria <descripcion>
    Given path "api/login/profesor-tutor-secundaria"
